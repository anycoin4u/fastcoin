...
UniValue getrawtransaction(const JSONRPCRequest& request)
{
    ...
    CTransactionRef tx;
    uint256 hashBlock;
    // Fastcoin: Is this the best value for consensus height?
...
UniValue createrawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx,    (numeric or string, required) The key is the fastcoin address, the numeric value (can be string) is the " + CURRENCY_UNIT + " amount\n"
...
UniValue decoderawtransaction(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"D731rRTrFydjJdZCKNzfB5go229p59GUGD\"   (string) fastcoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
