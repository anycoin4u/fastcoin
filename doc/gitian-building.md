...
*Setup instructions for a Gitian build of Fastcoin Core using a Debian VM or physical system.*
...
- [Building Fastcoin Core](#building-fastcoin-core)
...
Clone the git repositories for fastcoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/fastcoin/fastcoin.git
git clone https://github.com/fastcoin/gitian.sigs.git
```
...
in the fastcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Fastcoin Core
----------------

To build Fastcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fastcoin repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/fastcoin/fastcoin
...
For example:
```bash
URL=https://github.com/laanwj/fastcoin.git
...
and the fastcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fastcoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/fastcoin/fastcoin-detached-sigs.git

BTCPATH=/some/root/path/fastcoin
SIGPATH=/some/root/path/fastcoin-detached-sigs

./bin/gbuild --url fastcoin=${BTCPATH},signature=${SIGPATH} ../fastcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fastcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fastcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fastcoin-osx-build.assert
```
...
[fastcoin/gitian.sigs](https://github.com/fastcoin/gitian.sigs/) repository, or if that's not possible create a pull
