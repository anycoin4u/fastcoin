### Building fastcoin-qt 1.14 on modern macs. ###
...
### Clone fastcoin locally, or check it out, etc. ###

For this purpose, just indicating the 1.14-branding branch in my repo.

	$git clone -b 1.14-branding --single-branch https://github.com/michilumin/fastcoin.git
...
Patch bdb 5.1.29 from our patchfiles
    ...
    $cd src
    $cd dbinc
    $patch -b atomic.h ~/fastcoin/depends/patches/bdb-5.1.29-clang-osx/atomic.h.patch
    $cd ..
    $cd mp
    $patch -b mp_fget.c ~/fastcoin/depends/patches/bdb-5.1.29-clang-osx/mp_fget.c.patch
    $patch -b mp_mvcc.c ~/fastcoin/depends/patches/bdb-5.1.29-clang-osx/mp_mvcc.c.patch
    $patch -b mp_region.c ~/fastcoin/depends/patches/bdb-5.1.29-clang-osx/mp_region.c.patch
    $cd ..
    $cd mutex
    $patch -b mut_method.c ~/fastcoin/depends/patches/bdb-5.1.29-clang-osx/mut_method.c.patch
    $patch -b mut_tas.c ~/fastcoin/depends/patches/bdb-5.1.29-clang-osx/mut_tas.c.patch
